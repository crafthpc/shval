...
#define SH_ALLOC(V)     ;
#define SH_FREE(V)      ;
#define SH_SET(V,X)     (V) = f_to_p8e0(X)
#define SH_COPY(V,S)    (V) = (S)
#define SH_OUTPUT(O,V)  O << (float)p8e0_to_f(V)
#define SH_FINI         ;
...
#define SH_ADD(V,S)     p8e0_add(&(V),(V),(S))
#define SH_SUB(V,S)     p8e0_sub(&(V),(V),(S))
#define SH_MUL(V,S)     p8e0_mul(&(V),(V),(S))
#define SH_DIV(V,S)     p8e0_div(&(V),(V),(S))
#define SH_MIN(V,S)     (V) = (p8e0_gt((V),(S)) ? (S) : (V))
#define SH_MAX(V,S)     (V) = (p8e0_gt((V),(S)) ? (V) : (S))
#define SH_SQRT(V,S)    p8e0_sqrt(&(V),(S))
...
#define SH_DBL(V)       (double)(p8e0_to_f(V))
