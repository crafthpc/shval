...
#define SH_TYPE         p32e3_t
#define SH_INFO         "Posit(32,3) number"
#define SH_PARAMS       ""

#define SH_INIT         ;
#define SH_ALLOC(V)     ;
#define SH_FREE(V)      ;
#define SH_SET(V,X)     (V) = f_to_p32e3(X)
#define SH_COPY(V,S)    (V) = (S)
#define SH_OUTPUT(O,V)  O << (float)p32e3_to_f(V)
#define SH_FINI         ;
...
#define SH_ADD(V,S)     p32e3_add(&(V),(V),(S))
#define SH_SUB(V,S)     p32e3_sub(&(V),(V),(S))
#define SH_MUL(V,S)     p32e3_mul(&(V),(V),(S))
#define SH_DIV(V,S)     p32e3_div(&(V),(V),(S))
#define SH_MIN(V,S)     (V) = (p32e3_gt((V),(S)) ? (S) : (V))
#define SH_MAX(V,S)     (V) = (p32e3_gt((V),(S)) ? (V) : (S))
#define SH_SQRT(V,S)    p32e3_sqrt(&(V),(S))
...
#define SH_DBL(V)       (double)(p32e3_to_f(V))
