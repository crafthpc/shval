...
#define SH_TYPE         p16e2_t
#define SH_INFO         "Posit(16,2) number"
#define SH_PARAMS       ""

#define SH_INIT         ;
#define SH_ALLOC(V)     ;
#define SH_FREE(V)      ;
#define SH_SET(V,X)     (V) = f_to_p16e2(X)
#define SH_COPY(V,S)    (V) = (S)
#define SH_OUTPUT(O,V)  O << (float)p16e2_to_f(V)
#define SH_FINI         ;
...
#define SH_ADD(V,S)     p16e2_add(&(V),(V),(S))
#define SH_SUB(V,S)     p16e2_sub(&(V),(V),(S))
#define SH_MUL(V,S)     p16e2_mul(&(V),(V),(S))
#define SH_DIV(V,S)     p16e2_div(&(V),(V),(S))
#define SH_MIN(V,S)     (V) = (p16e2_gt((V),(S)) ? (S) : (V))
#define SH_MAX(V,S)     (V) = (p16e2_gt((V),(S)) ? (V) : (S))
#define SH_SQRT(V,S)    p16e2_sqrt(&(V),(S))
...
#define SH_DBL(V)       (double)(p16e2_to_f(V))
